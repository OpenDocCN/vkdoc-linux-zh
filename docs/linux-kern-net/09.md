...
![image](img/sq.jpg) **注**为了简单起见，[图 9-1](#Fig1) 没有包括更复杂的场景，比如使用 IPsec 或者分片或者组播的时候。为了简单起见，它还省略了在本地主机上生成并发送出去的数据包所调用的函数(如`ip_queue_xmit()`方法或`ip_build_and_send_pkt()`方法)。
