...
*   伦纳德的《脉冲音频和 Jack》( [`http://0pointer.de/blog/projects/when-pa-and-when-not.html`](http://0pointer.de/blog/projects/when-pa-and-when-not.html) )
...
`pavumeter`是一个简单的仪表，用于显示默认设备的输入或输出电平。当`pavumeter`运行时，显示播放设备，如图 6-4 所示。
...
如果由`pavumeter --record`运行，则显示记录设备，如图 6-5 所示。
...
`pavucontrol`允许您控制不同连接的音频设备的输入和输出音量，如图 6-6 所示。
...
Karaoke 文件由诸如`kmid`到`timidity`的 Karaoke 播放器播放。这里有一个例子:
...
当这两个运行时，可以使用`pavucontrol`控制相对音量，如图 6-7 。
...
命令`gnome-control-center sound`允许完全查看和控制连接的声音设备，包括选择默认输入和输出设备。看起来像图 6-8 。
