...
*   SourceArchive 的`fluidsynth`文档( [`http://fluidsynth.sourcearchive.com/documentation/1.1.5-1/main.html` ”)](http://fluidsynth.sourcearchive.com/documentation/1.1.5-1/main.html)
...
使用 ALSA 播放 MIDI 文件序列的典型程序如下。它创建各种对象，设置音频播放器使用 ALSA，然后将每个声音字体和 MIDI 文件添加到播放器中。然后对`fluid_player_play`的调用依次播放每个 MIDI 文件。该程序只是第二十章中所示程序的重复。
