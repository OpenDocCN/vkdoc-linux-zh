...
然后使用`AudioSystem`函数`write()`将流复制到文件中。图 9-2 显示了 UML 类图。
...
这是前两个节目的结合。准备一个`AudioInputStream`用于从麦克风读取。一个`SourceDataLine`是准备写给演讲者的。通过从音频输入流读取数据并写入源数据线，将数据从第一个复制到第二个。图 9-3 显示了 UML 类图。
...
在文件`PLATFORM_API_LinuxOS_ALSA_Ports.c`中，你会看到第五章中描述的对 ALSA 的函数调用。这些调用填充了供 JavaSound 使用的 ALSA 设备的信息。
