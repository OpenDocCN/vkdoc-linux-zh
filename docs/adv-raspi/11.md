...
图 [11-2](#Fig2) 显示了 GPIO 端口如何向其负载(显示为电阻)提供电流。电流从+3.3 V 电源流出，通过晶体管*M*<sub>1</sub>，流出 GPIO 引脚，进入*负载，然后*接地。因此，需要高电平(逻辑 1)才能将电流送入负载。这是一个*高电平有效*配置的例子。
